    ...
    inCollection: (v,p,propertyPath)=>{
        ...
        if(Array.isArray(v)) {
            for(const [key, id] of v.entries()) {
                if(!String(id).includes('$') && !p.widgets[id]) {
                    problems.push({
                        widget: p.widgetId,
...
const WIDGET_PROPERTIES = {
    ...
    Holder: {
        ...COMMON_PROPERTIES,
        movable: 'boolean', layer: 'number', dropTarget: 'any', dropOffsetX: 'number', dropOffsetY: 'number', dropShadow: 'any', alignChildren: 'any', preventPiles: 'any', allowPiles: 'any', childrenPerOwner: 'any', showInactiveFaceToSeat: 'any', onEnter: 'object', onLeave: 'object', layout: v=>v===null||['auto','custom','pile','singleSpread','multipleSpread','grid','freeform'].includes(v)||'layout must be auto, custom, pile, singleSpread, multipleSpread, grid or freeform', stackOffsetX: 'number', stackOffsetY: 'number', pilesOffsetX: 'numberOrNull', pilesOffsetY: 'numberOrNull', pilesGapX: 'numberOrNull', pilesGapY: 'numberOrNull', spreadMin: 'numberOrNull', gridColumns: 'numberOrNull', gridRows: 'numberOrNull', borderRadius: 'any', color: 'string', svgReplaces: 'any', text: 'any', textColor: 'any', icon: 'any', image: 'asset'
    },
    Label: {
        ...COMMON_PROPERTIES,
