...
app.use(config.HTTP_URL_PREFIX, router);
...
MinifyRoom().then(function(result) {
  ...
  router.use('/library', express.static(path.resolve() + '/library'));
  ...
  router.get('/share/:room/:state', function(req, res) {
    const target = `${config.HTTP_URL_PREFIX}/dl/${req.params.room}/${req.params.state}`;
    for(const link in sharedLinks)
      if(sharedLinks[link] == target)
        return res.send(link);

    const newLink = `${config.HTTP_URL_PREFIX}/s/${Math.random().toString(36).substring(3, 11)}`;
  ...
  router.put('/asset', bodyParser.raw({ limit: '100mb' }), function(req, res) {
