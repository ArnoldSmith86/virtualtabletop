...
export class Widget extends StateManaged {
  ...
  async evaluateRoutine(property, initialVariables, initialCollections, byReference) {
    ...
    for(const original of routine) {
      ...
      if(a.func == 'FOREACH') {
        ...
        const callWithAdditionalValues = async (addVariables, addCollections)=>{
          ...
          await this.evaluateRoutine(a.loopRoutine, variables, collections, true);
          for(const add in addVariables)
      ...
      if(a.func == 'IF') {
        ...
        if(a.condition !== undefined || a.operand1 !== undefined) {
          ...
          if(Array.isArray(a[branch]))
            await this.evaluateRoutine(a[branch], variables, collections, true);
        } else
  ...
  async onPropertyChange(property, oldValue, newValue) {
    if(property == 'parent') {
      ...
      if(newValue) {
        ...
        if(newParent.get('type') != 'holder' && Array.isArray(newParent.get('enterRoutine')))
