...
function gameFileInfo(json) {
  ...
  // an array is an object as far as typeof is concerned, and assigning one into a game spreads
  // it in as one property per element - server/room.mjs refuses it for the same reason
...
async function uploadStateFile(sourceFile, targetURL, metaCallback, progressCallback, loadCallback) {
  ...
  for(const [ filename, entry ] of Object.entries(entries)) {
    // the server reads a variant from every JSON file in the top level of the zip, so the JSON
    // of a PCIO or TTS file and anything an asset happens to contain is not one - but they do
    // make the archive one the server can turn into a game
    ...
    if(jsonFiles[filename]) {
      ...
      // and gameFileInfo hands back null for the info of one that holds something other than
      // the object everything reading a variant expects
