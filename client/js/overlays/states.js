...
async function uploadStateFile(sourceFile, targetURL, metaCallback, progressCallback, loadCallback) {
  ...
  // PCIO and TTS exports can keep their JSON outside the root of the archive.
  const containsJSON = Object.keys(entries).some(filename=>filename.match(/\.json$/));
  const convertedArchive = entries['widgets.json'] || entries['WorkshopUpload'];
  let info = null;
  const variants = [];
  const assets = {};
  for(const [ filename, entry ] of Object.entries(entries)) {
    if(!convertedArchive && jsonFiles[filename]) {
      const content = parseJSONorNull(jsonFiles[filename]);
      if(content && content._meta) {
        // A hand-written variant may have no usable metadata, but it still needs a row.
