...
function fillStatesList(states, starred, returnServer, activePlayers) {
    ...
    for (const publicLibrary of[false, true]) {
        ...
        for (const kvp of sortedStates.filter(kvp => (!!kvp[1].publicLibrary && (!starred || !starred[kvp[1].publicLibrary])) == publicLibrary)) {
            ...
            if (state.time) {
                $('.time', entry).textContent = state.time + " mins";
            } else {
                $('.time', entry).className += ' ui-hidden';
                $('.time-icon', entry).className += ' ui-hidden';
            }
            if (state.players) {
                $('.players', entry).textContent = state.players;
            } else {
                $('.players', entry).className += ' ui-hidden';
                $('.players-icon', entry).className += ' ui-hidden';
            }
            ...
            for (const variantID in state.variants) {
                ...
                $('.play', vEntry).addEventListener('click', _=>{ toServer('loadState', { stateID: state.id, variantID }); showOverlay(); $UI('#app').hide(); });
...
onLoad(function() {
    ...
    on('#home .navbar .download', 'click', _ => downloadState(null));
