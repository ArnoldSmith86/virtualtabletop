...
function commonInfoTopic(topicName) {
  ...
  if(topicName == 'SORT') {
    return { info: `
      ...
      groupBy: property name - only used on a holder with layout multipleSpread: the sorted widgets are partitioned into one spread group per distinct value of this property, e.g. one group per suit (defaults to null, which sorts the widgets within the existing groups instead).
