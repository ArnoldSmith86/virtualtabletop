...
const routineOperationMetadata = {
  ...
  MOVE: {
    ...
    clauses: [
      ...
      // where in the holder they end up: the pile a holder stacks its widgets
      // into, or the spread groups a multiple spread holder arranges
    ...
    parameters: {
      ...
      position: { type: 'enum', values: [ 'pileBottom', 'pileTop', 'groupStart', 'groupEnd' ], default: null,
        display: { pileBottom: 'at the bottom of the pile', pileTop: 'on top of the pile', groupStart: 'in a new spread group before the others', groupEnd: 'in a new spread group after the others' } }
    },
    ignored: (v, isSet)=>{
  ...
  SORT: {
    ...
    clauses: [
      ...
      // only a multiple spread holder has the groups this builds, so the option
      // says so rather than reading as something every sort can do
      { id: 'groupBy', label: 'into spread groups', template: ' and group them by {groupBy}' },
    ...
    parameters: {
      ...
      // one spread group per value this property has, in a multipleSpread holder
    ...
    ignored: v=>{
      ...
      // the groups are the ones a multiple spread holder arranges, so there are
      // none to build when the sort works on picked widgets instead of a holder
      else if(v('groupBy'))
        ignored.groupBy = 'ignored because only the widgets in a holder can be sorted into spread groups';
      return ignored;
    }
  },
  SWAPHANDS: {
