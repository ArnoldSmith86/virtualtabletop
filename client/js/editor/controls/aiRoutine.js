...
// A routine a deck hands to its cards is now checked twice over: the validator
// reads the deck's cardDefaults itself, and the made-up card carries the same
// routine. Both are about the same operation, so they are folded onto one shape -
// the deck, and a path that starts with the routine's own property, which is what
// ties a problem to the card it is warning about.
function aiCardProblem(problem, widgetID, property) {
  const path = Array.isArray(problem && problem.property) ? problem.property : [];
  const folded = { ...problem };
  if(problem && problem.widget === widgetID + AI_PROBE_CARD_SUFFIX)
    folded.widget = widgetID;
  if(path[0] == 'cardDefaults' && path[1] === property)
    folded.property = path.slice(1);
  return folded;
}
...
function aiValidateRoutine(widgetID, property, routine, target) {
  ...
  try {
    ...
    const fresh = after.filter(p=>!known.has(JSON.stringify(p)));
    if(target != 'cardDefaults')
      return fresh;
    const problems = [];
    const seen = new Set();
    for(const problem of fresh.map(p=>aiCardProblem(p, widgetID, property))) {
