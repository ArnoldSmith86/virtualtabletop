...
class DeckEditor {
  ...
  renderSidebar() {
    ...
    for(const property of Object.keys(object)) {
      ...
      // for every other type the engine writes it into the style attribute of that box (card.js).
      if(this.isCssProperty(property)) {
        this.addCssEditor(objectProps, property, {
          stateKey: `${this.deckID}:faceTemplates:${this.face}:${this.selectedObject}:${property}`,
          getValue: _=>object[property],
          ...
          allowClasses: object.type == 'html',
          defaultLabel: property == 'css' ? 'The object itself' : undefined,
          defaultInfo: object.type == 'html' ? 'Declarations applied to the object itself. Other sections are selectors matched inside its HTML.' : 'Declarations applied to the box this object is drawn in.',
