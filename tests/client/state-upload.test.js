...
describe('uploading a game file', () => {
  ...
  test('turns an image stored in the file into a data URL', async () => {
    ...
    const meta = await upload(overlay, vttFile('My Game.vtt', {
      '0.json': { _meta: { version: 8, info: { name: 'My Game', image: '/assets/1_2' } } },
      'assets/1_2': png
    }));
  ...
  test('ignores the asset map next to the variants', async () => {
    const overlay = loadStatesOverlay();
    const meta = await upload(overlay, vttFile('My Game.vtt', {
      '0.json': { _meta: { version: 8, info: { name: 'My Game' } } },
      'asset-map.json': { '1_2': 'assets/1_2' }
    }));

    expect(meta[0][0]).toBe('My Game');
  ...
  test('reports a file that is not a zip', async () => {
    const overlay = loadStatesOverlay();
    const meta = await upload(overlay, { name: 'notes.txt', arrayBuffer: async () => fflate.strToU8('not a zip').buffer });

    expect(meta).toEqual([]);
    expect(overlay.alerts).toEqual([ 'notes.txt does not contain a game. You can add VTT files (.vtt, .vttc, .vtts), playingcards.io files (.pcio) and Tabletop Simulator workshop files (.zip).' ]);
    expect(overlay.uploaded.length).toBe(0);
  });
