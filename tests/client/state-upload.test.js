...
describe('uploading a game file', () => {
  ...
  test('ignores the asset map next to the variants', async () => {
    ...
    expect(meta[0][3]).toHaveLength(1);
  });
  ...
  test('falls back to the file name when the root JSON has no metadata at all', async () => {
    const overlay = loadStatesOverlay();
    const meta = await upload(overlay, vttFile('Bare State.vtt', {
      '0.json': { w: { id: 'w' } }
    }));

    expect(meta[0][0]).toBe('Bare State');
    expect(overlay.alerts).toEqual([]);
  });
