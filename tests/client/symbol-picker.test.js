...
describe('the icon picker overlay', () => {
  ...
  test('a search says how it found nothing, and the outlined variant explains itself', async () => {
    ...
    // the tags are single, common English words, so the message also says what usually helps
    expect(document.getElementById('symbolNoResults').textContent).toBe('No icons match "nothing". Try a shorter or more common word.');
  });
  ...
  test('only the best 100 matches get the large previews, the rest goes below "More matches"', async () => {
    ...
    expect(picker.icon('icon_99').style.order).toBe('6'); // below the heading, which is 5
