...
test('Smart clone lifecycle', async t => {
  ...
  await ClientFunction(prepareClient)();
  // the Edit Widgets module holds the smart clone panel used below - remembering it
  // opens it together with edit mode, whether or not this window would get the
  // default one, so the panel is there without toggling the sidebar button
  await setEditorState({ modules: { 'Edit Widgets': 'editorModuleTopLeft' } });
  await setName(t);
