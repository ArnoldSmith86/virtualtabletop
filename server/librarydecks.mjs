...
// the 52 built-in French-suited card image names (AC..KS), as generated by the
// editor's standard deck; used to hide library copies of the standard deck
const standardFrenchCards = new Set();
for(const suit of [ 'C', 'D', 'H', 'S' ])
  for(const rank of [ 'A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K' ])
    standardFrenchCards.add(rank + suit);
const frenchJokerCards = new Set([ '1J', '2J', '3J', '4J' ]);

// true if the deck is just the standard 52-card French-suited deck (optionally
// with jokers): every card type uses a built-in /i/cards-default/ image and the
// non-joker images cover exactly the 52 standard cards
function isStandardFrenchDeck(deck) {
  const cardTypes = Object.values(deck.cardTypes || {});
  if(!cardTypes.length)
    return false;

  const cards = new Set();
  for(const cardType of cardTypes) {
    const match = typeof cardType.image == 'string' && cardType.image.match(/(?:^|\/)i\/cards-default\/([^/]+)\.svg$/);
    if(!match)
      return false;
    if(!frenchJokerCards.has(match[1]))
      cards.add(match[1]);
  }

  if(cards.size != standardFrenchCards.size)
    return false;
  for(const card of standardFrenchCards)
    if(!cards.has(card))
      return false;
  return true;
}
    ...
    for(const game of (await fs.promises.readdir(libraryDir)).sort((a,b)=>a.localeCompare(b))) {
      ...
      for(const file of files) {
        ...
        for(const [ widgetID, widget ] of Object.entries(state)) {
          ...
          // the editor can already add the standard deck, so don't list copies of it
          if(isStandardFrenchDeck(widget))
            continue;
